...
export const getStakedBalance = async function getStakedBalance() {
...
export const startStake = async function startStake(amount) {
  const account = await getAccount();
  const pool = await getInstance(StakingRewardPool);
  const cakeLP = await getInstance(CakeLP);
...
export const endStake = async function endStake(amount) {
  const account = await getAccount();
  const pool = await getInstance(StakingRewardPool);
  const cakeLP = await getInstance(CakeLP);
  const amountDecimals = await toTokenDecimals(cakeLP, amount);
...
export const claimReward = async function claimReward() {
  const account = await getAccount();
  const pool = await getInstance(StakingRewardPool);
  return new Promise(async (resolve, reject) => {
    try {
...
export const getRewardsStats = async function getRewardStats() {
  const pool = await getInstance(StakingRewardPool);
